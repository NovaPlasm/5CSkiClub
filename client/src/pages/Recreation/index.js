...
export default class Recreation extends React.Component {
    ...
    componentWillMount() {
        ...
        axios.get(`https://public-api.wordpress.com/rest/v1.1/sites/5cskisnowboard.home.blog/posts/?category=${type}`).then(data => {
            this.setState({
                recreation: data.data.posts.map((page) => ({
                    id: page.ID,
    ...
    render() {
        const { recreation } = this.state;

        if (recreation.length === 0) return null;

        return (
            <Div>
                <h1>{recreation[0].title}</h1>
                <p dangerouslySetInnerHTML={{__html: recreation[0].content}} />
            </Div>
        );
    }
}
